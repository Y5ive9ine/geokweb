...
export async function GET(
  ...
  try {
    ...
    // 构建真实API的URL
    const apiUrl = new URL(`${API_BASE_URL}/brands/${brandId}/geo-optimizations/recommendations`);
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          success: false,
          error: data.error || data.message || "Failed to generate GEO recommendations",
